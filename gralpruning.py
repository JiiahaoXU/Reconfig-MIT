...
'''
Credits:
This page is inspirsed by https://github.com/VITA-Group/GraNet/blob/main/CIFAR/sparselearning/core.py.
...
def add_sparse_args(parser):
    ...
    parser.add_argument('--reconfig_mit', action='store_true', help='Enable sparse mode. Default: True.')
    ...
    parser.add_argument('--init-density', type=float, default=0.50)
    parser.add_argument('--final-density', type=float, default=0.05)
    parser.add_argument('--init-prune-epoch', type=int, default=0)
    parser.add_argument('--final-prune-epoch', type=int, default=110)

    # parser.add_argument('--final-grow-epoch', type=int, default=110, help='The density of the overall sparse network.')
...
class Masking(object):
    def __init__(self, optimizer, prune_rate=0.3, growth_death_ratio=1.0, prune_rate_decay=None, death_mode='magnitude',
        ...
        self.steps = 0
        self.last_growing_ratio = 0

        self.pruning_stage = True

        if self.args.fix:
            self.prune_every_k_steps = None
            self.grow_every_k_steps = None
        else:
            self.prune_every_k_steps = self.args.update_frequency
            self.grow_every_k_steps = self.args.update_frequency
    ...
    def step(self):
        ...
        if self.grow_every_k_steps is not None and not self.pruning_stage:
            if self.steps >= (self.args.init_grow_epoch * len(
                    self.loader) * self.args.multiplier) and self.steps % self.grow_every_k_steps == 0:
                self.growing(self.steps)
    ...
    def pruning(self, step):
        ...
        if curr_prune_iter - ini_iter <= total_prune_iter:
            with open('./%s/pruning_log.txt' % self.log_root, 'a') as f:
                print('\n******************************************************', file=f)
                print(f'Pruning Progress is {curr_prune_iter - ini_iter} / {total_prune_iter}', file=f)
                print('******************************************************', file=f)

        if ini_iter <= curr_prune_iter <= final_iter - 1:
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    if name not in self.masks: continue
                    self.masks[name] = ((torch.abs(
                        weight)) > acceptable_score).float()  # must be > to prevent acceptable_score is zero, leading to dense tensors
            ...
            for name, weight in self.masks.items():
                total_size += weight.numel()
            with open('./%s/pruning_log.txt' % self.log_root, 'a') as f:
                print('Total Model parameters:', total_size, file=f)
            ...
            with open('./%s/pruning_log.txt' % self.log_root, 'a') as f:
                print('Sparsity after pruning: {0}'.format(
                    (total_size - sparse_size) / total_size), file=f)

    def growing(self, step):
        # prune_rate = 1 - self.args.final_density - self.args.init_density
        curr_grow_iter = int(step / self.grow_every_k_steps)
        final_iter = int(
            (self.args.final_grow_epoch * len(self.loader) * self.args.multiplier) / self.grow_every_k_steps)
        ini_iter = int(
            (self.args.init_grow_epoch * len(self.loader) * self.args.multiplier) / self.grow_every_k_steps)
        total_grow_iter = final_iter - ini_iter
        # if curr_prune_iter - ini_iter <= total_prune_iter:
        with open('./%s/growing_log.txt' % self.log_root, 'a') as f:
            print('\n******************************************************', file=f)
            print(f'Growing Progress is {curr_grow_iter - ini_iter} / {total_grow_iter}', file=f)
            print('******************************************************', file=f)
            # print('\n')

        if ini_iter <= curr_grow_iter <= final_iter + 1:
            grow_decay = (1 - ((curr_grow_iter - ini_iter) / total_grow_iter)) ** 3
            # curr_grow_rate = (1 - self.args.final_density) + (self.args.final_density - self.args.init_density) * (
            #         1 - grow_decay)
            curr_grow_rate = (1 - self.args.init_density) + (self.args.init_density - self.args.final_density) * (
                    1 - grow_decay)
            # print('current growing ratio:', curr_grow_rate)
            curr_sparsity = 1 - curr_grow_rate
            
            if curr_grow_iter == final_iter + 1:
                curr_sparsity = self.args.final_density
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    grad = grad * (self.masks[name] == 0).float()
                    weight_grad_abs.append(torch.abs(grad))

            # Gather all scores in a single vector and normalise
            all_scores = torch.cat([torch.flatten(x) for x in weight_grad_abs])
            ...
            num_params_to_grow = int((len(all_scores) * curr_sparsity))
            num_params_to_grow = num_params_to_grow - sparse_size
            # self.last_growing_ratio = curr_grow_rate
            # print('num_params_to_grow:', num_params_to_grow)
            ...
            for module in self.modules:
                for name, weight in module.named_parameters():
                    ...
                    new_mask = self.gradient_growth_ours(name, new_mask, acceptable_score, weight)
        ...
        with open('./%s/growing_log.txt' % self.log_root, 'a') as f:
            print('Density after growing: {0}'.format(
                1 - (total_size - sparse_size) / total_size), file=f)
    ...
    def pruning_uniform(self, step):
        ...
        if ini_iter <= curr_prune_iter <= final_iter:
            prune_decay = (1 - ((curr_prune_iter - ini_iter) / total_prune_iter)) ** 3
            curr_prune_rate = (1 - self.args.init_density) + (self.args.init_density - self.args.final_density) * (
                    1 - prune_decay)
    ...
    def add_module(self, module, sparse_init='ERK', grad_dic=None):
        ...
        if self.args.rm_first:
            for name, tensor in module.named_parameters():
                if 'model_tail.7.weight' in name or 'feature.0.weight' in name:
                    self.masks.pop(name)
    ...
    def gradient_growth_ours(self, name, new_mask, acceptable_score, weight):
        grad = self.get_gradient_for_weights(weight)
        grad = grad * (new_mask == 0).float()
        index = grad >= acceptable_score
        # y, idx = torch.sort(torch.abs(grad).flatten(), descending=True)
        # new_mask.data.view(-1)[idx[:total_regrowth]] = 1.0
        new_mask.data[index] = 1.0
