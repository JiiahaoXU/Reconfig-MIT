...
import numpy as np
import cv2
from gralpruning import Masking, CosineDecay


class Reconfig_MIT_Trainer():
        ...
        for epoch in range(self.args.epoch + 1, self.args.n_epochs + 1):
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                if self.args.bidirect:  # C dir
                    ...
                    # Recursive cycle-consistency loss (R-CCL).
                    if self.args.r_ccl:
                ...
                if self.args.r_ccl:
                    loss_dict['ex_loss'] = self.r_ccl
                self.logger.log(loss_dict, self.args.log_root)

            if epoch % self.args.eva_epoch == 0:
                self.netG_A2B.eval()
                with torch.no_grad():
                    MAE = 0
                    PSNR = 0
